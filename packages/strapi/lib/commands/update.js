    ...
    if (err) {
      ...
      _.forEach(config.generators, (info, name) => {
        try {
          ...
          logger.debug(
            'Cloning the `strapi-generate-' + name + '` repository for the first time...'
          );
          shell.exec(
            ...
            (code, stdout, stderr) => {
              if (code) {
                ...
                process.chdir(
                  path.resolve(__dirname, '..', 'node_modules', 'strapi-generate-' + name)
                );
                logger.debug('Installing dependencies for `strapi-generate-' + name + '`...');
                shell.exec('npm install', (code, stdout, stderr) => {
                  if (code) {
                    logger.error(
                      'Impossible to install dependencies for `strapi-generate-' + name + '`.'
                    );
                    console.log(stderr);
                  } else {
                    logger.info(
                      'Successfully installed dependencies for `strapi-generate-' + name + '`.'
                    );
                  }
                });
              }
