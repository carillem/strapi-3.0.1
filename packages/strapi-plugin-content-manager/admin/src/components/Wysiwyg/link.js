...
const Link = props => {
  ...
  if (includes(aInnerHTML, '<img', 'src=')) {
    ...
    const width = includes(aInnerHTML, 'width=')
      ? aInnerHTML.split('width="')[1].split('" ')[0]
      : '';
    const height = includes(aInnerHTML, 'height=')
      ? aInnerHTML.split('height="')[1].split('" ')[0]
      : '';
    content = (
      <img
        src={src}
        ...
        width={width}
        height={height}
        style={{ marginTop: '27px', maxWidth: '100%' }}
      />
    );
  }
