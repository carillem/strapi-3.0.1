...
export const cleanData = (retrievedData, ctLayout, groupLayouts) => {
  ...
  const recursiveCleanData = (data, layout) => {
    return Object.keys(data).reduce((acc, current) => {
      ...
      const isRepeatable = getOtherInfos(layout.schema, [current, 'repeatable']);
      let cleanedData;
          ...
          if (isRepeatable) {
            cleanedData = value
              ? value.map(data => {
                  ...
                  const subCleanedData = recursiveCleanData(data, groupLayouts[group]);

                  return subCleanedData;
                })
              : value;
          } else {
            cleanedData = value ? recursiveCleanData(value, groupLayouts[group]) : value;
          }
          break;
        default:
...
export const getMediaAttributes = (ctLayout, groupLayouts) => {
  const getMedia = (layout, prefix = '', isGroupType = false, repeatable = false) => {
...
export const mapDataKeysToFilesToUpload = (filesMap, data) => {
  return Object.keys(filesMap).reduce((acc, current) => {
    ...
    if (!isRepeatable) {
      const currentFilesToUpload = isMultiple ? getFilesToUpload(keys) : getFileToUpload([...keys]);
